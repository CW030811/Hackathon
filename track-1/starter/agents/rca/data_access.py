...
class CSVTelemetryStore:
    ...
    def component_catalog(self):
        ...
        # Logical scopes are derived only from observed pod membership. Raw query
        # resolution remains physical: a service never masquerades as one pod.
        services = {c.service for c in result.components.values() if c.kind == "container" and c.service}
        for service in sorted(services):
            members = [c for c in result.components.values() if c.kind == "container" and c.service == service]
            if service not in result.components:
                result.components[service] = Component(service, "service", service=service,
                    source_files=tuple(sorted({p for c in members for p in c.source_files})))
