...
def _family(kpi):
    ...
    # Collector spellings are not English substrings (r_s, rkb_s, etc.).
    if key in {"system_io_r_s", "system_io_rkb_s", "system_io_r_await"}:
        return "read_io"
    if key in {"system_io_w_s", "system_io_wkb_s", "system_io_w_await"}:
        return "write_io"
    if key == "system_disk_readonly":
        return "unknown"
...
def _hypothesis(kind, kpi, family, episode):
    ...
    if family == "memory" and not memory_auxiliary:
        if increasing != any(x in key for x in ("free", "available", "usable")):
            return "node memory consumption" if kind == "node" else "container memory load"
    if family in {"read_io", "write_io"} and increasing and "await" not in key:
    ...
    if kind == "node" and (key in {"system_disk_pct_usage", "system_disk_used", "system_disk_free"}
                           or any(x in key for x in ("disk_space", "disk_usage", "filesystem", "fs_usage"))):
...
def _resource_records(case, frame, query, coverage, families, deadline):
    ...
    for (component, raw_id, kpi), group in frame.dropna(subset=["_component_id", "kpi_name"]).groupby(["_component_id", "cmdb_id", "kpi_name"], sort=True):
        ...
        # CPU collector subtraction can leave ~1e-14 residue near zero. This is
        # a recorded numerical floor, not a universal native-unit threshold.
        if family == "cpu":
            params["thresholds"]["absolute_noise_floor"] = 1e-12
...
def compare_replicas_and_node(case, component, store, *, deadline):
    ...
    if target.kind == "service":
        members = [c for c in catalog.components.values() if c.kind == "container" and c.service == component]
        physical = sorted({c.component_id for c in members} | {c.node_id for c in members if c.node_id})
        return _analyse(case, store, ("metric_container", "metric_node"), tuple(physical), None, deadline)
    relations = {}
