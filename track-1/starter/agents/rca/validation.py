...
def validate_and_render(case, decision, evidence, catalog):
    ...
    for answer in answers:
        component = canonical.get(getattr(answer, "component", None))
        if component and component.kind == "service":
            members = {key for key, c in canonical.items() if c.kind == "container" and c.service == component.component_id}
            supported = {key for eid in decision.supporting_ids if eid in ledger
                         for key in ledger[eid].component_ids if key in members}
            nodes = {canonical[key].node_id for key in supported if canonical[key].node_id}
            if len(supported) < 2 or len(supported) * 2 <= len(members) or len(nodes) < 2:
                warnings.append(f"Service {component.component_id} lacks majority cross-node replica support; scope is an unverified best guess")
