...
def prepare_candidates(case, candidates, catalog, evidence):
    ...
    return groups + _service_scopes(case, groups, catalog)


def _service_scopes(case, candidates, catalog):
    """Explicit scope alternatives, not additional independent measurements.

    Require a majority of observed replicas, at least two nodes, and a common
    sampled onset interval for the same supported mechanism. This does not prove
    service causality; node and pod explanations remain in the candidate pool.
    """
    output = []
    for service, scope in sorted(catalog.components.items()):
        if scope.kind != "service":
            continue
        members = {key: c for key, c in catalog.components.items()
                   if c.kind == "container" and c.service == service}
        eligible = [c for c in candidates if c.component in members
                    and (not c.features.get("m4.weak_reason") and c.reason and c.onset_interval
                    or c.features.get("traces.network_family") and
                    _number(c.features, "traces.anomaly_score") > 0)]
        def interval(c):
            return c.onset_interval or (case.start, case.end)
        groups = []
        for c in sorted(eligible, key=lambda c: (c.reason or "", interval(c), c.candidate_id)):
            group = next((g for g in groups if g[0].reason == c.reason and
                bool(g[0].onset_interval) == bool(c.onset_interval) and
                max(interval(x)[0] for x in [*g, c]) <=
                min(interval(x)[1] for x in [*g, c])), None)
            if group is None:
                groups.append([c])
            else:
                group.append(c)
        for group in groups:
            affected = sorted({c.component for c in group})
            nodes = {members[key].node_id for key in affected if members[key].node_id}
            if len(affected) < 2 or len(affected) * 2 <= len(members) or len(nodes) < 2:
                continue
            representative = max(group, key=lambda c: (_feature_score(c.features), c.candidate_id))
            lower = max(interval(c)[0] for c in group)
            upper = min(interval(c)[1] for c in group)
            timed = all(c.onset_interval for c in group)
            parents = sorted(c.candidate_id for c in group)
            cid = stable_id(case.case_key, "m4.service_scope", {"parents": parents,
                "service": service, "transform": "service_scope.v1"})
            features = {**deepcopy(representative.features), "scope.members": affected,
                "scope.observed_members": sorted(members), "scope.nodes": sorted(nodes),
                "scope.coverage": len(affected) / len(members), "scope.onset_resolved": timed,
                "m4.provenance": parents}
            output.append(replace(representative, candidate_id=cid, component=service,
                onset_interval=(lower, upper) if timed else None,
                onset_estimate=lower + (upper - lower) / 2 if timed else None,
                features=features, supporting_ids=sorted({i for c in group for i in c.supporting_ids}),
                contradicting_ids=sorted({i for c in group for i in c.contradicting_ids}),
                unresolved=sorted({u for c in group for u in c.unresolved} | {
                    "Cross-node replica cochange supports service scope, but shared dependencies remain alternatives."}
                    | ({"Window-wide cochange has no localized onset; simultaneous failure is unverified."} if not timed else set())),
                episode_id=cid))
    return output
...
def _feature_parts(features):
    ...
    return {
        ...
        "service_scope": (.75 if features.get("scope.onset_resolved") else .25) * _number(features, "scope.coverage"),
    }
...
def _same_episode(left, right):
    ...
    if left.component == right.component:
        if left.onset_interval and right.onset_interval:
            return max(left.onset_interval[0], right.onset_interval[0]) <= min(left.onset_interval[1], right.onset_interval[1])
        return True
    if not separated and (left.component in right.features.get("scope.members", ()) or
                          right.component in left.features.get("scope.members", ())):
        return True
...
def make_decision(case, selected, ranked, catalog, *, warnings=(), stop_reason="best_guess",
    ...
    for component in sorted(catalog.components):
        ...
        if info.kind == "service":
            continue  # A logical scope requires evidence, not alphabetical fallback.
