...
def _log_values(frame, patterns, component=None, *, centered_matches=False, incident_start_s=None):
    baseline = None
    if incident_start_s is not None:
        before = _log_values(frame[frame["_timestamp_s"] < incident_start_s], patterns, component)
        baseline = {key: before[key] for key in ("searched_rows", "matched_rows", "pattern_match_counts")}
        frame = frame[frame["_timestamp_s"] >= incident_start_s]
    ...
    for index in selected.index[matched][:MAX_MATCH_SAMPLES]:
        ...
        sample = _positioned_text(row, found if centered_matches else ())
    ...
    if baseline is not None:
        ...
        baseline_rate = baseline["matched_rows"] / baseline["searched_rows"] if baseline["searched_rows"] else None
        incident_rate = int(matched.sum()) / len(selected) if len(selected) else None
        result["match_rate_delta"] = incident_rate - baseline_rate if incident_rate is not None and baseline_rate is not None else None
    return result


def _positioned_text(row, patterns=()):
    value = text(row.get("value"))
    record_index = number(row.get("_record_index"))
    positions = [value.casefold().find(p) for p in patterns if p in value.casefold()]
    start = max(0, min(positions) - 200) if positions else 0
    result = {"source_file": text(row.get("_source_file")),
            ...
            "log_name": text(row.get("log_name")), "text": value[start:start + MAX_TEXT_LENGTH],
            "text_truncated": len(value) > MAX_TEXT_LENGTH}
    if patterns:
        result.update(text_start_character=start, source_text_characters=len(value))
    return result
...
def search_fault_evidence(case: CaseContext, component_ids: tuple[str, ...],
    ...
    signature = (case.case_key, case.reference_start, case.start, case.end, components, normalized, ordered_sources)
    ...
    for source in ordered_sources:
        query = QuerySpec(source, case.reference_start, case.end, LOG_COLUMNS, component_ids=components)
        ...
        for component in observed_components or [None]:
            params = {"patterns": list(normalized), "component": component,
                      ...
                      "centered_matches": True, "incident_start_s": case.start.timestamp()}
            values = _log_values(frame, normalized, component, centered_matches=True,
                                 incident_start_s=case.start.timestamp())
            ...
            record = _evidence(case, "logs.literal_search.v3", params, values,
            ...
            record.interval = (case.reference_start, case.end)
            record.source_files = sorted(set(rows["_source_file"].dropna().astype(str)) - {""})
            bundle.evidence.append(record)
            if component and values["matched_rows"] and (values["match_rate_delta"] is None or values["match_rate_delta"] > 0):
                bundle.candidates.append(Candidate(
                    ...
                    features={"logs.matched_count": values["matched_rows"], "logs.anomaly_score": 0.5,
                              "logs.match_rate_delta": values["match_rate_delta"]},
...
def _replay_log_evidence(record: EvidenceRecord, store: TelemetryStore, *, deadline: float) -> dict:
    if record.transform not in ("logs.literal_search.v1", "logs.literal_search.v2", "logs.literal_search.v3"):
    ...
    frame, _, _, retained = _read_queries(record.queries, store, deadline, params["retained_rows_per_query"],
        memory_limit_bytes=params.get("memory_limit_bytes", LOG_MEMORY_BYTES) if not record.transform.endswith(".v1") else None)
    ...
    return _log_values(frame, tuple(params["patterns"]), params["component"],
                      centered_matches=params.get("centered_matches", False), incident_start_s=params.get("incident_start_s"))
