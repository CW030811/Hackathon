...
class SyntheticStoreTests(unittest.TestCase):
    ...
    def test_long_log_match_is_preserved_with_source_offset_and_replay(self):
        rows = self.logs()
        rows[1]["value"] = "x" * 4000 + " Timeout contacting dependency"
        self.write("log", "log_service", LOG_COLUMNS, rows)
        result = search_fault_evidence(self.case, ("pod-b",), ("timeout",), ("log_service",), self.store,
                                       deadline=time.monotonic() + 10)
        record = result.evidence[0]
        sample = record.values["match_samples"][0]
        self.assertIn("Timeout contacting dependency", sample["text"])
        self.assertGreater(sample["text_start_character"], 3000)
        self.assertEqual(record.values, replay_evidence(record, self.store, deadline=time.monotonic() + 10))

    def test_unchanged_background_errors_are_not_new_log_anomalies(self):
        rows = self.logs()
        for row in rows:
            row["value"] = "timeout"
        baseline = dict(rows[0], timestamp=self.start.timestamp() - 10, log_id="before")
        self.write("log", "log_service", LOG_COLUMNS, [baseline, *rows])
        result = search_fault_evidence(self.case, ("pod-b",), ("timeout",), ("log_service",), self.store,
                                       deadline=time.monotonic() + 10)
        self.assertEqual(result.evidence[0].values["match_rate_delta"], 0.)
        self.assertFalse(result.candidates)
        self.assertEqual(result.evidence[0].values, replay_evidence(result.evidence[0], self.store, deadline=time.monotonic() + 10))
