...
class ControllerTests(unittest.TestCase):
    ...
    def test_trace_competitor_not_hidden_by_stronger_metric(self):
        resource, trace = candidate(), candidate("trace", "trace-only-pod")
        trace.features = {"traces.anomaly_score": 2.}
        plan = _followup_plan([Ranked(resource, 7, {}), Ranked(trace, 2, {})], [])
        self.assertEqual(plan["tool"], "search_fault_evidence")
        self.assertIn("trace-only-pod", plan["components"])
    ...
    def test_weak_network_subtype_can_still_review_component_vs_resource(self):
        trace, resource = candidate(), candidate("resource", "resource-pod", eid="e2")
        trace.features = {"m4.weak_reason": True, "traces.anomaly_score": 3}
        resource.features = {"metrics.mechanism_supported": True}
        records = [evidence(kind="trace"), evidence("e2", "resource-pod")]
        ranked = [Ranked(resource, 6, {}), Ranked(trace, 3, {})]
        reply = dict(confidence="low", unresolved=["Component attribution is unresolved"])
        self.assertEqual(_escalation_reason(reply, [trace], ranked, records), "cross_source_or_scope_competition")
        records[1].coverage[0].status = "partial"
        self.assertIsNone(_escalation_reason(reply, [trace], ranked, records))
