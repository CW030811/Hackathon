...
class PromptBudgetTests(unittest.TestCase):
    def test_review_keeps_prior_choice_and_explicit_question(self):
        records = [evidence(f"e{i}", f"pod-{i}") for i in range(40)]
        ranked = [Ranked(candidate(f"c{i}", f"pod-{i}", eid=f"e{i}"), 40-i, {}) for i in range(40)]
        review = {"previous_candidate_ids": ["c39"], "question": "cross_source_or_scope_competition"}
        messages, offered, _ = build_messages(case(), ranked, records, stage="strong", review=review)
        self.assertIn("c39", {c.candidate_id for c in offered})
        self.assertEqual(json.loads(messages[1]["content"])["review"], review)
        self.assertIn("not a fresh blind vote", messages[0]["content"])
        self.assertLessEqual(prompt_bytes(messages), MAX_PROMPT_BYTES)

    def test_low_rank_trace_and_log_survive_metric_dominance(self):
        records, ranked = [], []
        for i in range(100):
            records.append(evidence(f"e{i}", f"pod-{i}"))
            ranked.append(Ranked(candidate(f"c{i}", f"pod-{i}", eid=f"e{i}"), 10, {}))
        for kind in ("trace", "log"):
            record = evidence(kind, kind + "-pod", kind=kind)
            c = candidate(kind, record.component_ids[0], eid=kind)
            c.features = {("traces" if kind == "trace" else "logs") + ".anomaly_score": 1.}
            records.append(record)
            ranked.append(Ranked(c, .5, {}))
        messages, offered, _ = build_messages(case(), ranked, records, stage="flash")
        self.assertTrue({"trace", "log"} <= {c.candidate_id for c in offered})
        self.assertGreaterEqual(len(offered), 12)
        omissions = json.loads(messages[1]["content"])["omissions"]
        self.assertEqual(omissions["shortlist_pruned"], 78)
        self.assertEqual(omissions["candidate_count"], len(ranked) - len(offered))
