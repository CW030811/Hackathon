...
class ValidationTests(unittest.TestCase):
    ...
    def test_observed_service_scope_needs_cross_node_member_support(self):
        self.catalog.components["pod"] = Component("pod", "service", service="pod")
        self.catalog.components["pod-8"] = Component("pod-8", "container", node_id="node-b", service="pod")
        self.decision.answers[0].component = "pod"
        self.assertEqual(self.render().validation_status, "degraded")
        other = deepcopy(self.record)
        other.evidence_id = "synthetic:other"
        other.component_ids = ["pod-8"]
        self.decision.supporting_ids.append(other.evidence_id)
        result = validate_and_render(self.case, self.decision, [self.record, other], self.catalog)
        self.assertEqual(result.validation_status, "valid")
