...
```python
Component:
    component_id: str                # 精确 node / pod 名称，或观察到的 service 关系名
    kind: str                        # node / container / service
    raw_ids: tuple[str, ...]
    node_id: str | None
    service: str | None
...
Service scope 扩展（M1/M2/M4/M5）：catalog 快照按已观察到的 container.service 建立 `kind=service` 条目，不改 raw_to_components 或原始 query 过滤语义。M2 对 service 补查显式展开为观察到的 pods 与 nodes。M4 仅为跨至少两个 node、过半且至少两个已观察 replica、同机制且 onset 区间相交的候选建立 service alternative；保留原 pod/node 候选，service 与其成员的同一 episode 不重复计故障。网络 subtype 仍为弱假设，聚合不能将它升级为已验证机制。M5 对 service 使用 container 原因集；缺少成员支持时降级而非声称已验证。目录覆盖有限，跨副本共变也可能来自共享依赖，并非服务根因证明。

M2 数值策略：CPU near-zero 的 `absolute_noise_floor=1e-12` 随 evidence.transform_params.thresholds 记录并参与 replay；旧记录不带该字段时保持旧行为。该阈值不用于其他 native-unit KPI。I/O throughput 与 await 分开：await 可支持读写延迟线索，但本身不确证负载机制；readonly 不属于 read I/O。CPU、memory、disk free/used 的机制判断包含方向，未经验证的 counter 不自动差分。

Trace-only service 候选是上述时间条件的显式弱例外：原观测只有整个查询窗口时保留 `onset_interval=None/onset_estimate=None`，记录“同时发生未验证”，给予较低 scope 加分，绝不把窗口起点当测得 onset。本发布组合不包含第 5 阶段，未知 onset 仍保留未知。

候选展示 v3：保留最低故障数的 episode 后，按 source/family/service-scope 分组轮流取样，每组优先不同 component，而非仅取全局 score 前 24。候选上限仍为 24；提示词上限 32,000 UTF-8 bytes（含 JSON 包装），证据卡保留标量、样本数、局部 onset 和日志命中位置，不复制完整 ledger。`candidate_funnel` 记录 prepared/shortlisted/offered 数、ID、提示词字节数及每阶段淘汰 ID，用于区分排序缺失和装载缺失；不把 score 当概率。

调查 v3：有独立 trace 竞争者时，优先对其和主资源候选做 transport/process 日志对照，而不是总是重查排名第一的资源；service 目标展开到观察到的 replicas。依赖深查只携带相关边（最多 12），仍保留 parent context；一次补查和既有总时限不变。日志 `logs.literal_search.v3` 使用 reference_start 到 end 查询，分别记录参考期与事件期的匹配行占比，背景错误占比未增加时不生成新 anomaly 候选。缺参考期不视为健康。命中摘要以关键词附近截取并保留原字符串偏移；v1/v2 replay 仍使用原参数行为。

复核 routing.v4：Strong 接收 Flash 的已选候选和未决事项，比较已观测的跨来源或 service/pod scope 竞争；上一轮选择保留在提示词中，但属于 model advice，不是测量事实。仅不可区分的 network subtype 不触发升级。记录固定 validator 错误类别；55 秒 case /1140 秒 run 和 thinking 配置不变。

本实验组合为 1、2、3、4、6：使用第 4 阶段的 metrics/traces 及证据视图，不包含第 5 阶段的多尺度 trace 窗口和短参考期 metrics。未知 trace onset 保留未知。
